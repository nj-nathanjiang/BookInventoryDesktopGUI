...
def search_command():
    list1.delete(0, END)
    for row in backend.search(title_entry.get(), author_entry.get(), year_entry.get(), rating_entry.get()):
...
def add_command():
    ...
    if title_entry.get() != "" and author_entry != "" and year_entry != "" and rating_entry != "":
        backend.insert(title_entry.get(), author_entry.get(), year_entry.get(), rating_entry.get())
...
def update_command():
    backend.update(selected_tuple[0], title_entry.get(), author_entry.get(), year_entry.get(), rating_entry.get())
